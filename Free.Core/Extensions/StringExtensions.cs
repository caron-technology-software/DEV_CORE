...
namespace Free.Core.Extensions.StringExtensions
{
    ...
    public static class StringExtensions
    {
        ...
        public static bool IsNumber(this string number)
        {
            ...
            if (number[0] == '-')
            {
                if (number.Substring(1).IsIntegerWithoutSign())
                {
                    return true;
                }
            ...
            // No sign
            if (number.IsIntegerWithoutSign())
            {
                return true;
            }
        ...
        static bool IsIntegerWithoutSign(this string integerWithoutSign)
        {
            ...
            if (integerWithoutSign.IndexOf("0x") == 0)
            {
                return integerWithoutSign.Substring(2).IsHexDigit();
            }
        ...
        static bool IsFloatWithoutSign(this string numberWithoutSign)
        {
            ...
            if (c < '0' || c > '9')
            {
                return false; // need at least one digit at the beginning
            }

            c = numberWithoutSign[1];
            if (c == '.')
            {
                if (numberWithoutSign.IsSimpleFloat())
                {
                    return true;
                }
            ...
            if (c == 'e' || c == 'E')
            {
                return numberWithoutSign.Insert(1, ".0").IsExpFloat();
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(numberWithoutSign))
            {
                return false;
            }
            ...
            while (token < len)
            {
                ...
                if (c == '.')
                {
                    ...
                    {
                        return false; // to many decimal points
                    }

                    if (token == len)
                    {
                        return false; // decimal points is not allowed at the end
                    }
        ...
        static bool IsExpFloat(this string numberWithoutSign)
        {
            ...
            if (c < '0' || c > '9')
            {
                return false; // need at least one digit after decimal point before the exponent starts
            }
            ...
            while (token < numberlen)
            {
                ...
                if (c == 'e' || c == 'E')
                {
                    ...
                    if ((numberlen - token) > 3)
                    {
                        return false;
                    }

                    if ((numberlen - token) == 0)
                    {
                        return false;
                    }
