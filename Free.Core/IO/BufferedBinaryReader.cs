...
namespace Free.Core.IO
{
    ...
    public class BufferedBinaryReader : IDisposable
    {
        ...
        public BufferedBinaryReader(Stream input, int bufferSize = 0x10000)
        {
            ...
            if (!input.CanRead)
            {
                throw new ArgumentException("Stream not readable.", nameof(input));
            }

            if (bufferSize <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(bufferSize), "Must be greater than zero.");
            }
        ...
        public virtual byte[] ReadBytes(int count)
        {
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(count), "Must not by smaller than zero (0).");
            }
        ...
        public virtual unsafe double ReadDouble()
        {
            ...
            if (len < fillsize)
            {
                Buffer.BlockCopy(m_buffer, 0, i_buffer, len, posInBuffer = fillsize - len);
            }
        ...
        public long Seek(long offset, SeekOrigin origin)
        {
            ...
            if (!m_stream.CanSeek)
            {
                throw new NotSupportedException("Stream not seekable.");
            }
