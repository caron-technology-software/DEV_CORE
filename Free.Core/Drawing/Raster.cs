...
namespace Free.Core.Drawing
{
    ...
    public class Raster<T> : TiledImage where T : struct, IEquatable<T>
    {
        ...
        public static int[] GenerateNeighboursXYOffsets(double radius, out int maxDistance)
        {
            ...
            {
                throw new ArgumentException("Must be a valid, non-infinte number.", nameof(radius));
            }
            ...
            for (var y = -rad; y <= rad; y++)
            {
                ...
                {
                    if (y * y + x * x > radiusAbs * radiusAbs)
                    {
                        continue;
                    }
        ...
        {
            if (null == neighboursXYOffsets)
            {
                throw new ArgumentNullException(nameof(neighboursXYOffsets));
            }
            ...
            if (neighboursXYOffsets.Length % 2 != 0)
            {
                throw new ArgumentException("Must have an even number of elements.", nameof(neighboursXYOffsets));
            }
        ...
        public Raster(int width, int height, bool init, int tileWidth = DefaultTileWidth, int tileHeight = DefaultTileHeight)
        {
            ...
            if (typeof(byte) == type)
            {
                ChannelTypes = ChannelTypeByte;
            }
            else if (typeof(ushort) == type)
            {
                ChannelTypes = ChannelTypeUShort;
            }
            else if (typeof(uint) == type)
            {
                ChannelTypes = ChannelTypeUInt;
            }
            else if (typeof(ulong) == type)
            {
                ChannelTypes = ChannelTypeULong;
            }
            else if (typeof(float) == type)
            {
                ChannelTypes = ChannelTypeSingle;
            }
            else if (typeof(double) == type)
            {
                ChannelTypes = ChannelTypeDouble;
            }
            else if (typeof(sbyte) == type)
            {
                ChannelTypes = ChannelTypeSByte;
            }
            else if (typeof(short) == type)
            {
                ChannelTypes = ChannelTypeSShort;
            }
            else if (typeof(int) == type)
            {
                ChannelTypes = ChannelTypeSInt;
            }
            else if (typeof(long) == type)
            {
                ChannelTypes = ChannelTypeSLong;
            }
            else
            {
                ChannelTypes = ChannelTypeStructure;
            }
        ...
        public Raster<T> GetTile(int x, int y, int channel)
        {
            if (channel == 0)
            {
                throw new ArgumentOutOfRangeException(nameof(channel), "Must be zero (0).");
            }
        ...
        public Raster<T> Dilation(int maxDistance, int[] neighboursInTileIndexOffsets, int[] neighboursXYOffsets)
        {
            if (maxDistance <= 0)
            {
                throw new ArgumentException("Must be greater than zero(0).", nameof(maxDistance));
            }

            if (null == neighboursInTileIndexOffsets)
            {
                throw new ArgumentNullException(nameof(neighboursInTileIndexOffsets));
            }
            ...
            if (neighboursXYOffsets.Length / 2 != neighboursInTileIndexOffsets.Length)
            {
                throw new ArgumentException("Must have twice as many elements than " + nameof(neighboursInTileIndexOffsets) + ".", nameof(neighboursXYOffsets));
            }
        ...
        public Raster<T> Erosion(int maxDistance, int[] neighboursInTileIndexOffsets, int[] neighboursXYOffsets, T newValue)
        {
            ...
            {
                throw new ArgumentException("Must be a non-default value.", nameof(newValue));
            }
                ...
                Parallel.For(0, amountY, line =>
                {
                    ...
                    for (int samp = 0; samp < amountX; samp++, index++)
                    {
                        if (fast && samp > dist && samp < amountX - dist) // We can use the neighboursInTileIndexOffsets when we're not at the border of the tile.
                        {
                            for (int i = 0; i < nitio.Length; i++)
                            {
                                structurElement[i] = orgTile[index + nitio[i]];
                            }
                        }
                        else
                        {
