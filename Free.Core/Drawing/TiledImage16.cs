    ...
    public class TiledImage16 : TiledImage
    {
        ...
        public long this[int x, int y]
        {
            get
            {
                if (x < 0 || x >= Width)
                {
                    throw new IndexOutOfRangeException(nameof(x) + " index must be greater or equal to zero (0), and smaller than the " + nameof(Width) + ".");
                }

                if (y < 0 || y >= Height)
                {
                    throw new IndexOutOfRangeException(nameof(y) + " index must be greater or equal to zero (0), and smaller than the " + nameof(Height) + ".");
                }
                ...
                {
                    return Bits[yTile * NumberOfTileX + xTile][yInTile * TileWidth + xInTile];
                }
                ...
                else
                {
                    ...
                    if (NumberOfChannels == 4)
                    {
                        ext = Bits[(yTile * NumberOfTileX + xTile) * NumberOfChannels + 3][yInTile * TileWidth + xInTile];
                    }
                }
        ...
        public ushort this[int channel, int x, int y]
        {
            get
            {
                ...
                {
                    throw new IndexOutOfRangeException(nameof(channel) + " index must be greater or equal to zero (0), and smaller than the " + nameof(NumberOfChannels) + ".");
                }
            ...
            set
            {
                ...
                {
                    Bits[(yTile * NumberOfTileX + xTile) * NumberOfChannels + channel][yInTile * TileWidth + xInTile] = value;
                }
            }
        }
        ...
        public TiledImage16(int width, int height, bool RGB, bool extraChannel, bool init, int tileWidth = 256, int tileHeight = 256, PlanarConfiguration planarConfig = PlanarConfiguration.Continuously)
        {
            if (width < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(width), "Must be greater zero (0).");
            }

            if (height < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(height), "Must be greater zero (0).");
            }

            if (tileWidth < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(tileWidth), "Must be greater zero (0).");
            }

            if (tileHeight < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(tileHeight), "Must be greater zero (0).");
            }

            if (extraChannel && !RGB)
            {
                throw new ArgumentException("Invalid channel setup. No extra channel, unless RGB.");
            }

            if (!RGB && planarConfig != PlanarConfiguration.Continuously)
            {
                throw new ArgumentException("Invalid planar configuration. Single-channel images must be continuously.", nameof(planarConfig));
            }
        ...
        public void Clear(params ushort[] color)
        {
            ...
            {
                throw new ArgumentException("Must contain a value for every channel.", nameof(color));
            }
            ...
            switch (NumberOfChannels)
            {
                ...
                case 3:
                    if (PlanarConfiguration == PlanarConfiguration.Continuously)
                    {
                        InitContinuousImageRGB(color[0], color[1], color[2]);
                    }
                    else
                    {
                        InitSeperatedImageRGB(color[0], color[1], color[2]);
                    }
                ...
                case 4:
                    if (PlanarConfiguration == PlanarConfiguration.Continuously)
                    {
                        InitContinuousImageRGBX(color[0], color[1], color[2], color[3]);
                    }
                    else
                    {
                        InitSeperatedImageRGBX(color[0], color[1], color[2], color[3]);
                    }
        ...
        public void Clear(long color = 0)
        {
            switch (NumberOfChannels)
            {
                ...
                case 3:
                    if (PlanarConfiguration == PlanarConfiguration.Continuously)
                    {
                        InitContinuousImageRGB((ushort)(color >> 32), (ushort)(color >> 16), (ushort)color);
                    }
                    else
                    {
                        InitSeperatedImageRGB((ushort)(color >> 32), (ushort)(color >> 16), (ushort)color);
                    }
                ...
                case 4:
                    if (PlanarConfiguration == PlanarConfiguration.Continuously)
                    {
                        InitContinuousImageRGBX((ushort)(color >> 32), (ushort)(color >> 16), (ushort)color, (ushort)(color >> 48));
                    }
                    else
                    {
                        InitSeperatedImageRGBX((ushort)(color >> 32), (ushort)(color >> 16), (ushort)color, (ushort)(color >> 48));
                    }
        ...
        public void ClearChannel(int channel, ushort value = 0)
        {
            if (channel < 0 || channel >= NumberOfChannels)
            {
                throw new ArgumentOutOfRangeException(nameof(channel) + " index must be greater or equal to zero (0), and smaller than the " + nameof(NumberOfChannels) + ".");
            }
            ...
            if (PlanarConfiguration == PlanarConfiguration.Continuously)
            {
                ...
                {
                    var tile = Bits[i];
                    for (int j = channel; j < tileSize; j += noc)
                    {
                        tile[j] = value;
                    }
                }
        ...
        public void Draw(int x, int y, TiledImage16 source, DrawBlendMode blendMode = DrawBlendMode.Overwrite)
        {
            ...
            if (x + source.Width <= 0 || x >= Width)
            {
                return;
            }

            if (y + source.Height <= 0 || y >= Height)
            {
                return;
            }
            ...
            {
                blendMode = DrawBlendMode.Overwrite;
            }
        ...
        public virtual TiledImage16 ExtraChannelRemoved()
        {
            ...
            if (this is Image16)
            {
                ret = new Image16(Width, Height, true, false, true, PlanarConfiguration);
            }
            else if (this is ChunkedImage16)
            {
                ret = new ChunkedImage16(Width, Height, true, false, true, ((ChunkedImage16)this).ChunkHeight, PlanarConfiguration);
            }
            else
            {
                ret = new TiledImage16(Width, Height, true, false, true, TileWidth, TileHeight, PlanarConfiguration);
            }
        ...
        public TiledImage16 GetSubImage(int x, int y, int width, int height)
        {
            ...
            if (this is Image16)
            {
                ret = new Image16(width, height, NumberOfChannels >= 3, NumberOfChannels > 3, true, PlanarConfiguration);
            }
            else if (this is ChunkedImage16)
            {
                ret = new ChunkedImage16(width, height, NumberOfChannels >= 3, NumberOfChannels > 3, true, ((ChunkedImage16)this).ChunkHeight, PlanarConfiguration);
            }
            else
            {
                ret = new TiledImage16(width, height, NumberOfChannels >= 3, NumberOfChannels > 3, true, TileWidth, TileHeight, PlanarConfiguration);
            }
        ...
        {
            if (x < 0 || x >= NumberOfTileX)
            {
                throw new ArgumentOutOfRangeException(nameof(x));
            }

            if (y < 0 || y >= NumberOfTileY)
            {
        ...
        public Image16 GetTile(int x, int y, int channel)
        {
            ...
            if (NumberOfChannels > 1 && PlanarConfiguration == PlanarConfiguration.Continuously)
            {
                ...
                var dst = ret.Bits[0];
                int noc = NumberOfChannels;
                for (int i = 0, a = channel; i < tileSize; i++, a += noc)
                {
                    dst[i] = src[a];
                }
            }
            ...
            {
                Buffer.BlockCopy(Bits[(y * NumberOfTileX + x) * NumberOfChannels + channel], 0, ret.Bits[0], 0, tileSize * sizeof(ushort));
            }
