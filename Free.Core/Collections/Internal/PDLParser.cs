    ...
    class PDLParser
    {
        ...
        Parameters ConsumeNextBlock()
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                if (c == '{')
                {
                    return ConsumeBlock(false); // Found start of the next block, so let's consume it and return the result.
                }
                ...
                {
                    throw new TextParserException("Invalid character.", line);
                }
            }
        ...
        Parameters ConsumeBlock(bool consumeSemicolon)
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                if (c == '}')
                {
                    if (!consumeSemicolon)
                    {
                        return ret; // No semicolon to read, so we're done here.
                    }
            ...
            while (!stream.EndOfStream)
            {
                ...
                if (c == ';')
                {
                    return ret; // End of the block-parameter (Key={...};).
                }
        ...
        string ConsumeParameterName(char c0)
        {
            ...
            if (c0 == '"')
            {
                ret.Append(ConsumeStringWithoutSemicolon());
            }
            else
            {
                ret.Append(c0);
            ...
            while (!stream.EndOfStream)
            {
                ...
                {
                    ret.Append(ConsumeStringWithoutSemicolon()); // Consume as regular string. Invalid characters will be detected be Parameter.Add().
                }
                else
                {
                    throw new TextParserException("unexpected character while parsing a name", line);
                }
            }
        ...
        void ConsumeBoolFalse(bool consumeSemicolon)
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                else if (letter == 2 && (c == 'L' || c == 'l'))
                {
                    letter++;
                }
                else if (letter == 3 && (c == 'S' || c == 's'))
                {
                    letter++;
                }
        ...
        void ConsumeNumber(char c0, Parameters ret, string name)
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                if (c == '+' || c == '-' || (c >= '0' && c <= '9') || c == '.' || c == 'x' || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'))
                {
                    numberSB.Append(c);
                }
                else
        ...
        string ConsumeVerbatimStringWithoutSemicolon()
        {
            ...
            while (!stream.EndOfStream)
            {
                ...
                if (c == '"')
                {
                    ...
                    if (a != '"')
                    {
                        return ret.ToString(); // Closing double-quote.
                    }
        ...
        void ConsumeBoolList(char c0, Parameters ret, string name)
        {
            ...
            {
                throw new ArgumentException("Must be either 't', 'T', 'f' or 'F'.", nameof(c0));
            }
        ...
        void ConsumeStringList(char c0, Parameters ret, string name)
        {
            ...
            {
                throw new ArgumentException("Must be either '\"' or '@'.", nameof(c0));
            }
