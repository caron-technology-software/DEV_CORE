...
namespace Free.Core.Collections.Internal
{
    ...
    static class ParametersToPDLConverter
    {
            ...
            for (int i = 0; i < names.Length; i++)
            {
                ...
                if (cooked)
                {
                    ...
                    try
                    {
                        ...
                        {
                            val = DataBlobToPDL((byte[])value, true);
                        }
                        else if (value is Parameters)
                        {
                            val = ParametersToPDL((Parameters)value, true, newIndent);
                        }
                        else if (value is List<string>)
                        {
                            val = StringListToPDL((List<string>)value, true, newIndent);
                        }
                        else if (value is List<Parameters>)
                        {
                            val = ParametersListToPDL((List<Parameters>)value, true, newIndent);
                        }
                        else if (value is List<long>)
                        {
                            val = IntegerListToPDL((List<long>)value, true, newIndent);
                        }
                        else if (value is List<double>)
                        {
                            val = FloatListToPDL((List<double>)value, true, newIndent);
                        }
                        else if (value is List<bool>)
                        {
                            val = BooleanListToPDL((List<bool>)value, true, newIndent);
                        }
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            val = DataBlobToPDL((byte[])value, false);
                        }
                        else if (value is Parameters)
                        {
                            val = ParametersToPDL((Parameters)value);
                        }
                        else if (value is List<string>)
                        {
                            val = StringListToPDL((List<string>)value, false, indent);
                        }
                        else if (value is List<Parameters>)
                        {
                            val = ParametersListToPDL((List<Parameters>)value, false, indent);
                        }
                        else if (value is List<long>)
                        {
                            val = IntegerListToPDL((List<long>)value, false, indent);
                        }
                        else if (value is List<double>)
                        {
                            val = FloatListToPDL((List<double>)value, false, indent);
                        }
                        else if (value is List<bool>)
                        {
                            val = BooleanListToPDL((List<bool>)value, false, indent);
                        }
        ...
        static string DataBlobToPDL(byte[] data, bool cooked)
        {
            ...
            if (len == 0)
            {
                throw new ArgumentException("Array must not be empty.", nameof(data));
            }
