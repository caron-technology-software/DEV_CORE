...
namespace Free.Core.Collections
{
    ...
    public class Parameters : ICloneable, IEquatable<Parameters>, IEnumerable
    {
        ...
        static int ParseListIndex(string name, out string listname)
        {
            ...
            if (name.Length < 4)
            {
                return -1; // 4: name+index+brackets
            }

            if (name[name.Length - 1] != ']')
            {
                return -1; // Last char must be ']'.
            }

            int index = name.IndexOf('[');
            if (index < 1)
            {
                return -1; // Nothing found, or string begins with '['.
            }

            string indexString = name.Substring(index + 1, name.Length - (index + 2));

            foreach (char ch in indexString)
            {
                if (ch < '0' || ch > '9')
                {
                    return -1; // Only digits allowed, otherwise return -1.
                }
            }

            int ret = 0;
            if (!int.TryParse(indexString, out ret))
            {
                return -1;
            }
            ...
            foreach (string arg in args)
            {
                ...
                if (arg[0] == '-')
                {
                    ...
                    if (index >= 0)
                    {
                        ...
                        {
                            ret.Add(key, arg.Substring(index + 1));
                        }
                    }
                    else
                    {
        ...
        void Put(string path, object value)
        {
            ...
            if (key.Length == 0)
            {
                throw new ArgumentException(InvalidNameMessage, nameof(path));
            }
            ...
            if (parameter != null)
            {
                parameter.Put(subKey, value);
            }
            else
            {
                PutIntoNewSubParameters(key, value);
            }
        }
        ...
        void PutEntry(string name, object value)
        {
            ...
            if (index == -1)
            {
                ...
                if (name.Length > 2)
                {
                    nameBrackets = name.Substring(name.Length - 2);
                }
                ...
                if (nameBrackets == "[]")
                {
                    ...
                    if (!CheckName(nameWithoutBrackets))
                    {
                    ...
                    if (items.ContainsKey(nameWithoutBrackets))
                    {
                        ...
                        if (iList != null)
                        {
                            ...
                            {
                                throw new ArgumentException(TypeMismatchMessage, nameof(value));
                            }
            ...
            unchecked
            {
                ...
                else
                {
                    ...
                    foreach (string key in keys)
                    {
                        ...
                        if (v == null)
                        {
                            hashCode = GetHash((Parameters)null, hashCode);
                        }
                        else if (v is bool)
                        {
                            hashCode = GetHash((bool)v, hashCode);
                        }
                        else if (v is double)
                        {
                            hashCode = GetHash((double)v, hashCode);
                        }
                        else if (v is long)
                        {
                            hashCode = GetHash((long)v, hashCode);
                        }
                        else if (v is string)
                        {
                            hashCode = GetHash((string)v, hashCode);
                        }
                        else if (v is Parameters)
                        {
                            hashCode = GetHash((Parameters)v, hashCode);
                        }
                        else
                        {
                            hashCode = Math.Abs(hashCode * 1234597);
                        }
                    }
                }
        ...
        public object Clone()
        {
            ...
            foreach (string key in items.Keys)
            {
                ...
                else if (obj is List<Parameters>)
                {
                    ...
                    foreach (Parameters element in list)
                    {
                        value.Add((Parameters)element.Clone()); // Clone of the Parameters in the List<Parameters>.
                    }
        ...
        public bool Equals(Parameters other)
        {
            ...
            foreach (string key in items.Keys)
            {
                ...
                {
                    if (!(objectB is string))
                    {
                        return false;
                    }

                    if ((string)objectA != (string)objectB)
                    {
                        return false;
                    }
                }
                ...
                {
                    if (!(objectB is Parameters))
                    {
                        return false;
                    }

                    if (!((Parameters)objectA).Equals((Parameters)objectB))
                    {
                        return false;
                    }
                }
