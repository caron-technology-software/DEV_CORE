...
namespace Free.Core.Collections
{
    ...
    public class Bitfield : ICollection, IEnumerable, IEnumerable<bool>
    {
        ...
        public bool Get(int bit)
        {
            if (bit < 0 || bit >= length)
            {
                throw new ArgumentOutOfRangeException(nameof(bit), "Must be equal or greater than 0 and less than Length.");
            }
        ...
        public bool Get(uint bit)
        {
            if (bit >= length)
            {
                throw new ArgumentOutOfRangeException(nameof(bit), "Must be less than Length.");
            }
        ...
        public void Set(int from, int to)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(from), "Must be equal or greater than 0 and less than Length.");
            }

            if (to < 0 || to >= length)
            {
                throw new ArgumentOutOfRangeException(nameof(to), "Must be equal or greater than 0 and less than Length.");
            }
        ...
        public void Set(uint from, uint to)
        {
            if (from >= length)
            {
                throw new ArgumentOutOfRangeException(nameof(from), "Must be less than Length.");
            }

            if (to >= length)
            {
                throw new ArgumentOutOfRangeException(nameof(to), "Must be less than Length.");
            }
            ...
            {
                uint update = 0;
                for (int i = bit; i < 32 && count > 0; i++, count--)
                {
                    update |= 1u << i;
                }
            ...
            {
                uint update = 0;
                for (int i = 0; i < 32 && count > 0; i++, count--)
                {
                    update |= 1u << i;
                }
        ...
        public void CopyTo(Array destination, int index)
        {
            ...
            if (index < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(index), "Must not be less than zero (0).");
            }

            if (destination.Rank != 1)
            {
                throw new ArgumentException("Must be a 1-dimensional array.", nameof(destination));
            }

            bool[] dst = destination as bool[];
            if (dst == null)
            {
                throw new NotSupportedException("Only bool[] supported.");
            }

            if ((long)dst.Length - index < length)
            {
                throw new ArgumentException("Array destination (starting at index) is not long enough.");
            }

            for (int i = 0; i < length; i++)
            {
                dst[index + i] = (bitfield[i / 32] & (1u << (i % 32))) != 0;
            }
        }
