...
namespace Free.Crypto
{
    ...
    public static class Adler32
    {
        ...
        public static uint GetSlow(IList<byte> data, int offset = 0, int count = 0, uint adler32 = 1)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("count", "Must be less than or equal to the length of data minus the offset argument.");
            }
            ...
            if (sum1 >= 65521 || sum2 >= 65521)
            {
                throw new ArgumentException("adler32", "Not a valid Adler-32-Checksum.");
            }
        ...
        public static uint Combine(uint adler1, uint adler2, ulong length2)
        {
            ...
            if (sum1a >= 65521 || sum2a >= 65521)
            {
                throw new ArgumentException("adler1", "Not a valid Adler-32-Checksum.");
            }

            uint sum1b = adler2 & 0xffff;
            uint sum2b = adler2 >> 16;

            if (sum1b >= 65521 || sum2b >= 65521)
            {
                throw new ArgumentException("adler2", "Not a valid Adler-32-Checksum.");
            }

            if (length2 == 0)
            {
                throw new ArgumentException("length2", "Must not be zero.");
            }
        ...
        public static uint GetParallel(byte[] data, int offset = 0, int count = 0, uint adler32 = 1)
        {
            ...
            if (count < 2 * BytesPreThread)
            {
                return Get(data, offset, count, adler32);
            }
            ...
            uint ret = adler32;
            for (int i = 0; i < threadsMinus1; i++)
            {
                ret = Combine(ret, adler32s[i], BytesPreThread);
            }
