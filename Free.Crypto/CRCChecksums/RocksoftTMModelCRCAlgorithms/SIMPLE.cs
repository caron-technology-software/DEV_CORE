...
namespace Free.Crypto.CRCChecksums.RocksoftTMModelCRCAlgorithms
{
    ...
    public static class SIMPLE
    {
        ...
        public static uint Get(int width, uint polynomial, IList<byte> data, int offset = 0, int count = 0)
        {
            ...
            if (offset + count > data.Count * 8)
            {
                throw new ArgumentOutOfRangeException("count", "Must be less than or equal to the length of data in bits minus the offset argument.");
            }
            ...
            if (polynomial == 0)
            {
                throw new ArgumentOutOfRangeException("polynomial", "Must not be 0 in the relevant bits.");
            }
            ...
            while (offset < end)
            {
                ...
                if (offset < endOfBitStream && ((data[byteInArray] & bitRot) != 0))
                {
                    register |= 1;
                }
                ...
                if (pop)
                {
                    register ^= polynomial; // Register = Register XOR Poly.
                }
