...
namespace Free.Crypto.CRCChecksums
{
    public static partial class CRC
    {
        ...
        public static ICRC<byte> Create(byte polynomial, byte init = 0, bool refIn = false, bool refOut = false, byte xorOut = 0, int width = 8)
        {
            if (refIn)
            {
                return new ReflectedByte(polynomial, init, refOut, xorOut, width);
            }
        ...
        public static ICRC<ushort> Create(ushort polynomial, ushort init = 0, bool refIn = false, bool refOut = false, ushort xorOut = 0, int width = 16)
        {
            if (refIn)
            {
                return new ReflectedUShort(polynomial, init, refOut, xorOut, width);
            }
        ...
        public static ICRC<uint> Create(uint polynomial, uint init = 0, bool refIn = false, bool refOut = false, uint xorOut = 0, int width = 32)
        {
            if (refIn)
            {
                if (width == 32 && polynomial == 0x04C11DB7)
                {
                    return new CRC32(init, refOut, xorOut);
                }
        ...
        public static ICRC<ulong> Create(ulong polynomial, ulong init = 0, bool refIn = false, bool refOut = false, ulong xorOut = 0, int width = 64)
        {
            if (refIn)
            {
                return new ReflectedULong(polynomial, init, refOut, xorOut, width);
            }
        ...
        public static ICRC<UInt128> Create(UInt128 polynomial, UInt128 init, bool refIn = false, bool refOut = false)
        {
            if (refIn)
            {
                return new ReflectedUInt128(polynomial, init, refOut, UInt128.Zero, 128);
            }
        ...
        public static ICRC<UInt128> Create(UInt128 polynomial, UInt128 init, bool refIn, bool refOut, UInt128 xorOut, int width = 128)
        {
            if (refIn)
            {
                return new ReflectedUInt128(polynomial, init, refOut, xorOut, width);
            }
        ...
        {
            if (descriptor.Width <= 0 || descriptor.Width > 128)
            {
                throw new ArgumentOutOfRangeException("descriptor.Width", "Must be greater than 0 and less than 129.");
            }

            if (descriptor.Width <= 8)
            {
                return Create((byte)descriptor.Polynomial, (byte)descriptor.Init, descriptor.RefIn, descriptor.RefOut, (byte)descriptor.XorOut, descriptor.Width);
            }

            if (descriptor.Width <= 16)
            {
                return Create((ushort)descriptor.Polynomial, (ushort)descriptor.Init, descriptor.RefIn, descriptor.RefOut, (ushort)descriptor.XorOut, descriptor.Width);
            }

            if (descriptor.Width <= 32)
            {
                return Create((uint)descriptor.Polynomial, (uint)descriptor.Init, descriptor.RefIn, descriptor.RefOut, (uint)descriptor.XorOut, descriptor.Width);
            }

            if (descriptor.Width <= 64)
            {
                return Create((ulong)descriptor.Polynomial, (ulong)descriptor.Init, descriptor.RefIn, descriptor.RefOut, (ulong)descriptor.XorOut, descriptor.Width);
            }
