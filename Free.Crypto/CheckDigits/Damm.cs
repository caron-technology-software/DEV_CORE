...
namespace Free.Crypto.CheckDigits
{
    ...
    public static class Damm
    {
        ...
        public static char GetCheckDigit(string digits, int[,] TAQ = null)
        {
            ...
            else if (TAQ.GetLength(0) != 10 || TAQ.GetLength(1) != 10)
            {
                throw new ArgumentException("Must be an 2D int array with both dimensions 10, or null.", "TAQ");
            }
        ...
        public static int GetCheckDigit(int[] digits, int[,] TAQ = null)
        {
            ...
            foreach (var ch in digits)
            {
                if (ch < 0 || ch > 9)
                {
                    throw new ArgumentOutOfRangeException("digits", "Must be an array of single digit numbers");
                }
